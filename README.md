...
*   **RateLimiter:** set a service `AddRateLimiter` to prevent the high rate hits the api while i have free subscription api version from [IPGeolocation.io] and depend on its documentations i have only 1000 reqest per day so i set the limit through GeoGuard to 100 per minute so you will be availbale to used features as `Lookup` and `CheckBlock()` ony for 1000 git per day. so should put this situation when used these specific features.
